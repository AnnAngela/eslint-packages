...
- **`@annangela/eslint-config` (major)**：升级 `@eslint/js` 从 v9 到 v10，引入 3 个新推荐规则：
...
- **`@annangela/eslint-config` (major)**: Upgraded `@eslint/js` from v9 to v10, introducing 3 new recommended rules:
  ...
  - Restored the `name` property on `@eslint/js/recommended` config
