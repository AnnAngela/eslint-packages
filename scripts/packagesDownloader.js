...
for (const packageName of [
    ...
    const { "dist-tags": { latest }, versions: { [latest]: { dist: { tarball } } } } = await (await fetch(`https://registry.npmjs.org/${packageName}`)).json();
    let version;
    try {
    ...
    if (version === latest) {
        ...
        const tmp = path.join(os.tmpdir(), `${packageName}.tgz`);
